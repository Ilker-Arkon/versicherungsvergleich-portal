...
- [x] (31.08.) **Performance — preconnect Partner-Rechner:** `preconnect` zu `form.partner-versicherung.de` + `dns-prefetch` zu `a.partner-versicherung.de` im Root-Layout (`preconnect`/`prefetchDNS` aus `react-dom`, Next.js-16-Konvention). Spart ~300–500 ms (DNS/TCP/TLS) beim ersten Kontakt zum Live-Rechner. TS-Check grün.
